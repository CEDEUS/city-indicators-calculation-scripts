...
walksheds.carpeta <- "E:/Cedeus Sustainability Indicators/GIS/Areas Verdes/Walksheds (Manzanas)/UnDissolved"
accesibilidad.carpeta <- "E:/Cedeus Sustainability Indicators/GIS/Areas Verdes/Resultados"
...
for (c in seq(ciudades)){
  ...
  walksheds.shp <- readOGR(walksheds.carpeta, paste(ciudad, "_Walksheds_10min (All_UnDissolved)", sep = ""), encoding = "UTF-8") %>%
    spTransform(proj4string(areas_verdes.shp))
  # Los walksheds que tienen contacto con un área verde
  walksheds.subset <- walksheds.shp[areas_verdes.shp,]
  ...
  accesibilidad.shp@data[accesibilidad.shp@data$ID_W %in% 
                           walksheds.subset@data$ID,]$Acceso <- "Sí"
  accesibilidad.shp@data[!(accesibilidad.shp@data$ID_W %in% 
                             walksheds.subset@data$ID),]$Acceso <- "No"
...
write.csv(accesibilidad.tabla, paste(tabla_accesibilidad.carpeta,"Accesibilidad ", ciudad, "(Población).csv", sep = ""))
write.csv(accesibilidad.tabla_porcentajes, paste(tabla_accesibilidad.carpeta,"Accesibilidad ", ciudad, "(Porcentajes).csv", sep = ""))
# Limpiar el entorno
rm(list=ls())
